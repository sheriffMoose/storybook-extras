const { readFileSync } = require('fs');
const { compile } = require('@storybook/mdx2-csf');
const { getCompileOptions } = require('./getCompileOptions');

const mdxCode = (title, content) => `

import { Description, Meta } from '@storybook/blocks';

<Meta title='${title}' />

${content}

`;

const _toMDX = (fileName, addonOptions) => {
    // Read the file
    const content = readFileSync(fileName, 'utf8');
    // Get the compile options
    const options = getCompileOptions(fileName, addonOptions);
    // Get the title
    const title = options.makeTitle(fileName);

    const code = mdxCode(title, content);

    return { code, options };
};

exports.getMDX = async function (fileName, addonOptions, toMDX = _toMDX) {
    // Generate MDX code
    const { code, options } = toMDX(fileName, addonOptions);

    // Compile MDX into CSF
    const csfCode = await compile(code, options);

    return csfCode;
};
