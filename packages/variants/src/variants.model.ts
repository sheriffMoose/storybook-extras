...
export class Variants {
  ...
  constructor(
    private storyFn: LegacyStoryFn | any,
    private context: StoryContext | any
  ) {
    this._init();
  }
  ...
  isEnabled() {
    const { globals, parameters } = this.context;
    ...
    const isPredefined = (config?.include || []).length;
  ...
  getTemplate() {
    ...
    const concatenatedVariants = concatDeep(
      mappedVariants,
      groupSeperator,
      variantSeperator
    );
  ...
  private _getCombinations(): any[] {
    ...
    const argsMap = {};
    const sortedArgTypes = [...Object.entries(this.context.argTypes)].sort(
